...
class thuvien{
	...
	public:
		...
		int Sua();
		int xoasach();
		bool kiemtraid(string idt){
			for(int j=0;j<B.size() ;j++){
				if(B.at(j)->getid() == idt) return true;
...
void thuvien::adddanhsach(){
	...
	for(int i=0;i<this->n;i++){
        ...
        tv->add();
        int kt=0;
        if(kiemtraid(tv->getid()) == true ){
        	cout<<"\nID bi trung vui long nhap lai !!!";
        	--i;
		}
		else{
		B.push_back(tv);
...
int main(){
    ...
    cout  << setw( 42 ) << "( N ) Nhap Danh sach Sach"
		  ...
		  << setw( 48 ) << "( K ) The Doc Het Han (tam thoi bi loi)"<< endl << endl
